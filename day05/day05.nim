...
when isMainModule:
  ...
  suite "day5 part1 challenge":
    ...
    test "check":
      check:
        "input.txt".readFileToIntSeq().process(inputs = @[idAirConditionerUnit]).outputs[^1] == 10987514
  ...
  suite "day5 part2 challenge":
    ...
    test "check":
      check:
        "input.txt".readFileToIntSeq().process(inputs = @[idThermalRadiatorController]).outputs[^1] == 14195011
