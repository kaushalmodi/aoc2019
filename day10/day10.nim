...
type
  Coord = tuple
    ...
    y: int
  Map = Table[Coord, int]
  AngleMap = OrderedTable[float, tuple[coord: Coord, dist: float]]
...
proc updateAsteroidsDetCounts(map: var Map; loc: Coord): AngleMap =
  for coord in map.keys:
    ...
    when defined(debug):
      echo &"{coord} :: {angle} {dist}"
    if not result.hasKey(angle) or
       result.hasKey(angle) and dist < result[angle].dist:
      result[angle] = (coord, dist)

  # Sort the AngleMap type table by the angle keys.
  result.sort(cmp)

  when defined(debug3):
    echo &"From {loc} asteroid:"
    for angle in result.keys:
      echo &"  {result[angle].coord} at {angle}"
  let
...
proc getVaporizedAsteroids(map: var Map; loc: Coord): seq[Coord] =
  ...
  while true:
    let
      angleMap = mapCopy.updateAsteroidsDetCounts(loc)
    mapCopy.del(loc) # Remove the current asteroid from the map
    for key, val in angleMap.pairs:
